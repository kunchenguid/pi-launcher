...
Checks, in order:
  ...
  7. release-please bootstrap-sha stays at the historical start commit, the
     manifest last-released version matches the highest published bare vX.Y.Z
     tag (never a hardcoded launcher version), tags have no component prefix,
     the human path stays, and autonomous merge stays gated
...
# Historical start commit for release-please. Moving this skips or rewrites
# version history; it is the only hardcoded release-please anchor.
RELEASE_PLEASE_BOOTSTRAP_SHA = "431fb3ae841dbb46ac81105b72eb5c62c5b6f997"
BARE_VERSION_RE = re.compile(r"^(0|[1-9]\d*)\.(0|[1-9]\d*)\.(0|[1-9]\d*)$")
...
def latest_published_launcher_version(tags):
    """Highest published launcher version from bare vX.Y.Z git tags.

    Component-prefixed tags such as pi-launcher-v1.2.0 are ignored: they are
    not a release.yml tag and must not become the contract's published version.
    """
    versions = []
    for tag in tags:
        if not isinstance(tag, str) or not tag.startswith("v"):
            continue
        version = tag[1:]
        if BARE_VERSION_RE.fullmatch(version):
...
def release_please_anchor_status(manifest, config, tags):
    """Whether release-please is still anchored without a frozen version.

    The 2026-08-15 upstream Pi gate failed because this contract required
    manifest == 1.2.0 after launcher 1.2.1 had already published. Quarantine
    then blocked Pi v0.84.2 forever. The last-released version must come from
    published tags so a future launcher release cannot recreate that trap.
    """
    if config.get("bootstrap-sha") != RELEASE_PLEASE_BOOTSTRAP_SHA:
        return (
            False,
            f"bootstrap-sha={config.get('bootstrap-sha')!r} "
            f"expected={RELEASE_PLEASE_BOOTSTRAP_SHA}",
        )
    if config.get("packages", {}).get(".", {}).get("release-type") != "simple":
        return False, "release-type is not simple"
    if not isinstance(manifest, dict) or set(manifest) != {"."}:
        return False, f"manifest={manifest!r}"
    last = manifest.get(".")
    if not isinstance(last, str) or BARE_VERSION_RE.fullmatch(last) is None:
        return False, f"manifest version {last!r} is not X.Y.Z"
    published = latest_published_launcher_version(tags)
    if published is None:
        return False, "no published vX.Y.Z tags"
    if last != published:
        return False, f"manifest={last} latest published={published}"
    return True, f"v{published}"
...
def main():
    ...
    anchor_ok, anchor_detail = release_please_anchor_status(
        release_please_manifest,
        release_please_config,
        git_v_tags(ROOT),
    )
    check(
        "release-please manifest matches the latest published vX.Y.Z tag",
        anchor_ok,
        anchor_detail,
    )
