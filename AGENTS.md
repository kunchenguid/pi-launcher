...
- `scripts/` - fetch/build/sign/verify pipeline; `check-release-contract.py` is the secret-free CI gate for workflow/packaging changes. The autonomous-update scripts (`update-pi-pin.py`, `merge-release-pr.py`, `pi-quarantine.py`) share `gh_api.py` and keep all discovery, merge-guard, and quarantine logic out of workflow YAML.
